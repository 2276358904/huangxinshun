...
flags.DEFINE_string("vocab_file", None, "The vocabulary file that the modeling was trained on.")
    ...
    for input_file in input_files:
        with tf.io.gfile.GFile(input_file, "r") as reader:
            while True:
                ...
                # If line is None, which means at the end of document, break it.
                if not line:
                    break
                line = line.strip()
...
def create_instances_from_document(
    ...
    # to `max_seq_length` anyway, so short sequences are generally wasted
    ...
    # We DON'T just concatenate all tokens from a document into a long
...
def create_masked_lm_predictions(tokens, masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
    ...
    for (i, token) in enumerate(tokens):
        if token == "[CLS]" or token == "[SEP]":
            continue
        # Whole Word Masking means that if we mask all the word pieces
        ...
        # at all -- we still predict each WordPiece independently, softmax
...
def main(_argv):
    ...
    input_files = []
    for input_patten in FLAGS.input_file.split(","):
        input_files.extend(tf.io.gfile.glob(input_patten))
